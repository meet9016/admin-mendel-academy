...
function AddSubjectForm() {
  ...
  const fetchSubjectById = async () => {
    try {
      const { data } = await api.get(`${endPointApi.getByIdSubject}/${id}`);
      setFormData({
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (id) {
        const res = await api.patch(`${endPointApi.updateSubject}/${id}`, formData);
        toast.success(res.data?.message || "Subject updated successfully");
      } else {
        const res = await api.post(`${endPointApi.createSubject}`, formData);
  ...
  return (
    <div className="space-y-6">
      ...
      <ComponentCard title={id ? "Edit Subject" : "Add Subject"} name="">
        <div className="space-y-6">
          <div>
          ...
          <div className="flex items-center gap-5">
            ...
            <Button size="sm" variant="outline" onClick={() => router.push("/question-bank")}>
              Cancel
            </Button>
          </div>
        </div>
